...
import { 
  ...
  type ThreatAlert as APIThreatAlert,
  type NetworkStatus as APINetworkStatus,
  type VulnerabilityData as APIVulnerabilityData,
  type IncidentData as APIIncidentData,
  type ThreatIntelligenceResult as APIThreatIntelligenceResult
} from '@/lib/api-client';

// Local types extending API types
interface ThreatAlert extends APIThreatAlert {}
interface NetworkStatus extends APINetworkStatus {}
interface VulnerabilityData extends APIVulnerabilityData {}
interface IncidentData extends APIIncidentData {}
interface ThreatIntelligenceResult extends APIThreatIntelligenceResult {}
...
export function RealtimeSOCDashboard() {
  ...
  const [activeScan, setActiveScan] = useState<VulnerabilityData | null>(null);
  const [analysisResult, setAnalysisResult] = useState<ThreatIntelligenceResult | null>(null);
  ...
  useEffect(() => {
    ...
    const socket = setupRealtimeHandlers(
      (alert: ThreatAlert) => {
  ...
  const handleAnalyzeIndicator = () => {
    if (indicatorToAnalyze.trim()) {
      analyzeThreatIndicator(indicatorToAnalyze.trim(), 'auto');
